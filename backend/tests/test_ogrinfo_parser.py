...
async def test_parses_well_formed_geojson_dump(fm, tmp_path):
    with patch(
        "filter_manager.asyncio.create_subprocess_exec",
...
async def test_recognizes_all_gdal_types(fm, tmp_path):
    ...
    with patch(
        "filter_manager.asyncio.create_subprocess_exec", return_value=_proc_with_stdout(text)
    ):
        fields = await fm._get_fields(tmp_path / "x")
    assert fields == [
        "a_str",
...
async def test_empty_output_returns_empty_list(fm, tmp_path):
    with patch("filter_manager.asyncio.create_subprocess_exec", return_value=_proc_with_stdout("")):
